...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_self_attn_mask, dec_self_attn_mask, dec_enc_attn_mask, enc_dec_attn_mask,):
        ...
        enc_outputs, enc_self_attn = self.dec_self_attn(enc_inputs, enc_inputs, enc_inputs, attn_mask=enc_self_attn_mask)
        
        dec_outputs, dec_self_attn = self.dec_self_attn(dec_inputs, dec_inputs, dec_inputs, attn_mask=dec_self_attn_mask)

        #enc_out, enc_enc_attn = self.dec_enc_attn(enc_outputs, dec_outputs, dec_outputs, attn_mask=enc_dec_attn_mask)
        
        #dec_out, dec_enc_attn = self.dec_enc_attn(dec_outputs, enc_outputs, enc_outputs, attn_mask=dec_enc_attn_mask)
        

        #with open("demo.txt", "a") as f:
            #f.write(str(enc_self_attn[0]) + "\n" + str(enc_self_attn.size()))

        #print(enc_outputs.size(), dec_outputs.size(), enc_out.size(), dec_out.size())

        enc_outputs = self.pos_ffn(enc_outputs) # enc_out
        
        dec_outputs = self.pos_ffn(dec_outputs) # dec_out
